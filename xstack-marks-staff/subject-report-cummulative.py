...
workbook_name = os.path.join(os.path.join(dirname, "mm-xport-generated"),
                             'GeneratedSubjectCumMMReport_' + random_string(8) + '.xlsx').replace('\\\\', '/')
...
def mysql_connection(subject_code, department, semester, exam):
    ...
    try:
        ...
        prepare_workbook(worksheet, subject_code, department,
                         semester, sub_det, col_name, exam, exams)
        row_no = 7
...
def prepare_workbook(worksheet, subject_code, department, semester, sub_det, col_name, exam, exams):
    global workbook, file_head_format, column_head_format

    worksheet.merge_range('A1:BZ1', 'Marks Module : Subject Reports - Cummulative - Loyola-ICAM College of Engineering and Technology',
                          file_head_format)
    ...
    worksheet.merge_range('A2:BZ2', "Subject Code: " + subject_code + "| Year: " +
                          year + "| Semester: " + str(semester)+" | Exam: "+exam.strip("'").upper(), file_head_format)
